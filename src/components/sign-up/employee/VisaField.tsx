...
export default function VisaField() {
  ...
  const filteredList = useMemo(() => {
    ...
    return VISA_LIST.filter((visa) => visa.label.toLowerCase().includes(searchValue.toLowerCase()))
  }, [searchValue])

  // 현재 선택된 비자의 라벨을 찾는 함수
  const getSelectedLabel = () => {
    if (!employeeSignUp?.visa) return '비자를 선택해주세요.'
    const found = VISA_LIST.find((visa) => visa.code === employeeSignUp.visa)
    return found?.label || employeeSignUp.visa
  }
    ...
    <div className="flex flex-col gap-y-2">
      ...
      <div onClick={handleInputContainerClick} className="relative">
        {isDropBoxOpen ? (
          ...
          >
            <p className={`${employeeSignUp?.visa ? 'text-black' : 'text-gray4'} button`}>{getSelectedLabel()}</p>
        ...
        {isDropBoxOpen && (
          <section className="border-gray2 absolute top-14 z-10 flex max-h-[420px] w-full flex-col overflow-y-auto rounded-[16px] border bg-white shadow-md">
            {filteredList.length > 0 ? (
              ...
              <div className="text-gray4 flex h-[60px] items-center justify-center px-4">검색 결과가 없습니다.</div>
            )}
          </section>
        )}
      </div>
