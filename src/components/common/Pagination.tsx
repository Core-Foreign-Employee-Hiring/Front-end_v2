...
const Pagination: React.FC<PaginationProps> = ({ currentPage, totalPages, onPageChange, showPages = 5 }) => {
  ...
  return (
    <nav className="flex items-center justify-center gap-1" aria-label="페이지네이션">
      ...
      >
        {currentPage === 1 ? (
          <Gray2LeftArrowIcon width={7} height={14} />
        ) : (
          <Gray3LeftArrowIcon width={7} height={14} />
        )}
      </button>
      ...
      {pageNumbers.map((page) => (
        <button
          ...
          className={`bg-main-light text-gray3 flex h-[32px] min-w-[32px] items-center justify-center rounded-full transition-colors duration-200 ${
            page === currentPage ? 'bg-main-light text-main' : 'hover:bg-gray-20 bg-white'
          } `}
          aria-label={`페이지 ${page}`}
      ...
      >
        {currentPage === totalPages ? (
          <Gray2RightArrowIcon width={7} height={14} />
        ) : (
          <Gray3RightArrowIcon width={7} height={14} />
        )}
      </button>
    </nav>
  )
